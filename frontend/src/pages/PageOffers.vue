<template>
  <q-page class="q-pa-md q-gutter-sm flex flex-center">
    ...
    <div v-if="offersDownloaded" class="q-pa-md row items-center justify-around q-gutter-md">
...
export default {
  ...
  methods: {
    ...
    async updateOffer () {
      console.log('update')
      try {
        const res = await this.$axios.get(`offer/${this.currentOffer.guid}`)
    ...
    async deleteImage () {
      ...
      try {
        const res = await this.$axios.delete(`/image/${this.currentOffer.guid}`, {
          data: params
        })
        console.log(res)
    ...
    async updateImageDisplay () {
      const files = this.$refs.imgCapture.files
    ...
    async downloadOffers () {
      try {
        this.$q.loading.show()
        const res = await this.$axios.get('/offers')
        this.$q.loading.hide()
        console.log(res)
